...
class UnifiedCGMProcessor:
    ...
    def process(self, csv_path: str, device_type: str) -> pd.DataFrame:
        ...
        if device_type not in self.SUPPORTED_DEVICES:
            raise ValueError(f"Unsupported device: {device_type}. Use: {self.SUPPORTED_DEVICES}")
    ...
    def _parse_medtronic_g4(self, csv_path: str) -> pd.DataFrame:
        ...
        df = df.rename(columns={
            'Date': 'timestamp_raw',
            'Sensor Glucose (mg/dL)': 'glucose_raw'
        })
    ...
    def _standardize(self, df: pd.DataFrame, device_type: str) -> pd.DataFrame:
        ...
        df['glucose_mgdl'] = pd.to_numeric(df['glucose_raw'], errors='coerce')
    ...
    def _fill_gaps(self, df: pd.DataFrame, max_gap_min: int = 15) -> pd.DataFrame:
        ...
        and should be flagged, not filled, to avoid fabricating data.
        """
        df = df.copy()
        df = df.set_index('timestamp').sort_index()

        # Expected 5-minute intervals
        expected = pd.date_range(start=df.index.min(), end=df.index.max(), freq='5min')
        df = df.reindex(expected)

        df['gap_minutes'] = df.index.to_series().diff().dt.total_seconds() / 60
        df['gap_flag'] = df['gap_minutes'] > max_gap_min

        max_interpolate_points = max_gap_min // 5
        df['glucose_mgdl'] = df['glucose_mgdl'].interpolate(
            method='linear', 
            limit=max_interpolate_points,
    ...
    def compute_metrics(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        in_range = (glucose >= self.time_in_range_low) & (glucose <= self.time_in_range_high)
        tir = in_range.mean() * 100

        in_tight = (glucose >= self.time_in_tight_range_low) & (glucose <= self.time_in_tight_range_high)
