...
from scipy.stats import ttest_rel, wilcoxon, pearsonr, pointbiserialr
...
class MultiLanguageContentManager:
    ...
    TRANSLATIONS = {
        'filipino': {
            'en': {
                ...
                'mods': ['Add grilled chicken before rice', 'Use cauliflower rice mix', 
                        'Add okra and eggplant', 'Use coconut vinegar']
            },
            'es': {
                'title': 'Adobo Filipino: Tradicional y Amigable con la Glucosa',
                'mods': ['Agregue pollo a la parrilla', 'Use mezcla de arroz de coliflor',
                        'Agregue okra y berenjena', 'Use vinagre de coco']
            },
            'tl': {
                'title': 'Adobong Filipino: Tradisyonal at Glucose-Friendly',
                'mods': ['Magdagdag ng inihaw na manok', 'Gumamit ng cauliflower rice',
                        'Magdagdag ng okra at talong', 'Gumamit ng suka ng niyog']
            }
        },
        'indian': {
            'en': {
                'title': 'Indian Dal & Roti: Traditional & Glucose-Friendly',
                'mods': ['Add paneer or tofu', 'Use chickpea flour roti',
                        'Add spinach and fenugreek', 'Use cinnamon and turmeric']
            }
        },
        'vietnamese': {
            'en': {
                'title': 'Vietnamese Pho: Traditional & Glucose-Friendly',
                'mods': ['Add extra protein', 'Use thin rice noodles',
                        'Load up on bean sprouts', 'Squeeze lime before eating']
            },
            'vi': {
                'title': 'Pho Viet Nam: Truyen Thong & Than Thien voi Duong Huyet',
                'mods': ['Them protein', 'Su dung bun mi mong',
                        'Nhieu gia va rau thom', 'Vat chanh truoc khi an']
            }
        },
        'latinx': {
            'en': {
                'title': 'Latinx Pozole: Traditional & Glucose-Friendly',
                'mods': ['Add shredded chicken early', 'Use hominy in moderation',
                        'Top with avocado', 'Use fresh cilantro and oregano']
            }
        }
    }
    ...
    QUIZ_TRANSLATIONS = {
        'q1': {
            'en': {'text': 'Adding protein before carbohydrates will:', 
                   'options': {1: 'Increase spike', 2: 'Blunt spike', 3: 'No effect', 99: "Don't know"}},
            'es': {'text': 'Agregar proteina antes de carbohidratos:', 
                   'options': {1: 'Aumenta pico', 2: 'Reduce pico', 3: 'Sin efecto', 99: 'No se'}},
            'tl': {'text': 'Ang pagdagdag ng protein bago ang carbs ay:', 
                   'options': {1: 'Tataas ang spike', 2: 'Bababa ang spike', 3: 'Walang epekto', 99: 'Hindi alam'}}
        },
        'q2': {
            'en': {'text': 'Thin rice noodles vs. thick flat noodles:', 
                   'options': {1: 'Higher GI', 2: 'Lower GI', 3: 'Same GI', 99: "Don't know"}},
            'es': {'text': 'Fideos de arroz delgados vs. gruesos:', 
                   'options': {1: 'Mayor IG', 2: 'Menor IG', 3: 'Igual IG', 99: 'No se'}},
            'vi': {'text': 'Bun mi mong vs. bun mi day:', 
                   'options': {1: 'GI cao hon', 2: 'GI thap hon', 3: 'GI nhu nhau', 99: 'Khong biet'}}
        }
    }

    def get_content(self, cuisine: str, lang: str = 'en') -> Dict:
        """Retrieve translated content for a cuisine module."""
        return self.TRANSLATIONS.get(cuisine, {}).get(lang, 
               self.TRANSLATIONS.get(cuisine, {}).get('en', {}))

    def get_quiz(self, q_id: str, lang: str = 'en') -> Dict:
        """Retrieve translated quiz question."""
        return self.QUIZ_TRANSLATIONS.get(q_id, {}).get(lang,
               self.QUIZ_TRANSLATIONS.get(q_id, {}).get('en', {}))
...
class EnhancedQuizScorer:
    ...
    def score_quiz(self, row, prefix="pre") -> Dict:
        ...
        for q_id, correct in self.answer_key.items():
            ...
            if col_name in row.index:
                ...
                details[q_id] = {'answer': answer, 'correct': correct, 'is_correct': is_correct}
        return {
            'score': score, 
    ...
    def process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df['pre_score'] = df.apply(lambda row: self.score_quiz(row, 'pre')['score'], axis=1)
        df['post_score'] = df.apply(lambda row: self.score_quiz(row, 'post')['score'], axis=1)
    ...
    def generate_item_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for q_id in self.answer_key.keys():
            ...
            if pre_col in df.columns and post_col in df.columns:
                ...
                pre_pb = pointbiserialr(pre_binary, df['pre_score'])[0] if 'pre_score' in df.columns else None
                post_pb = pointbiserialr(post_binary, df['post_score'])[0] if 'post_score' in df.columns else None
...
class MixedEffectsAnalyzer:
    ...
    def _wide_to_long(self, df: pd.DataFrame, id_col: str, time_col: str,
                      value_cols: List[str], time_mapping: Dict) -> pd.DataFrame:
    ...
    def fit_knowledge_model(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            model = mixedlm("score ~ timepoint * C(module_cuisine)", 
                           long_df, 
        ...
        return {
            ...
            'random_effects_variance': result.cov_re.iloc[0, 0] if len(result.cov_re) > 0 else None,
    ...
    def fit_self_efficacy_model(self, df: pd.DataFrame) -> Dict:
        ...
        long_data = []
        for _, row in df.iterrows():
            if all(c in row.index for c in ['mi_confidence_recipes', 
                                             'mi_confidence_cooking', 
                                             'mi_confidence_family']):
                se = (row['mi_confidence_recipes'] + 
                      row['mi_confidence_cooking'] + 
                      row['mi_confidence_family']) / 3
                long_data.append({
        ...
        try:
            model = mixedlm("self_efficacy ~ timepoint * C(module_cuisine) + readiness + importance",
                           long_df,
                           groups=long_df["participant_id"])
            result = model.fit()
...
class BayesianAnalyzer:
    ...
    def estimate_knowledge_gain(self, pre_scores: np.ndarray, post_scores: np.ndarray,
        ...
        try:
            ...
            with pm.Model() as model:
                mu_pre = pm.Normal('mu_pre', mu=pre_scores.mean(), sigma=pre_scores.std())
                mu_post = pm.Normal('mu_post', mu=post_scores.mean(), sigma=post_scores.std())
                sigma = pm.HalfNormal('sigma', sigma=max(pre_scores.std(), post_scores.std()))
                ...
                gain = pm.Deterministic('gain', mu_post - mu_pre)
            ...
            return {
                ...
                'probability_positive': 1 - stats.norm.cdf(0, loc=mean_diff, scale=se_diff),
...
class OutcomeLinker:
    ...
    def link_and_analyze(self, education_df: pd.DataFrame, 
                         biomarker_df: pd.DataFrame,
                         join_cols: List[str] = None) -> Dict:
        ...
        if missing_edu or missing_bio:
            return {'error': f'Missing columns. Education: {missing_edu}, Biomarker: {missing_bio}'}
        ...
        return {
            ...
            'n_linked': len(merged),
            'linkage_rate': len(merged) / len(education_df) if len(education_df) > 0 else 0
        }
...
class InterventionReportGenerator:
    ...
    def generate(self, df: pd.DataFrame, output_dir: str = ".") -> Dict:
        ...
        se_cols = ['mi_confidence_recipes', 'mi_confidence_cooking', 'mi_confidence_family']
        ...
        if 'knowledge_gain' in df.columns:
            ...
            cuisine_summary = df.groupby('cuisine_label')['knowledge_gain'].agg(['mean', 'std', 'count']).reset_index()
            ax.bar(cuisine_summary['cuisine_label'], cuisine_summary['mean'],
                   yerr=cuisine_summary['std']/np.sqrt(cuisine_summary['count']),
                   capsize=5, color=colors)
            ax.set_title('Knowledge Gain by Module (Mixed Effects Adjusted)', fontsize=14)
            ax.set_ylabel('Mean Knowledge Gain (points)')
            plt.tight_layout()
            figs['knowledge_gain'] = fig
            plt.savefig(f"{output_dir}/pb2_knowledge_gain.png", dpi=150, bbox_inches='tight')
            plt.close()

        # Plot 2: Self-efficacy pre/post
        if 'self_efficacy_pre' in df.columns and 'self_efficacy_post' in df.columns:
            fig, ax = plt.subplots(figsize=(10, 6))
            efficacy_summary = df.groupby('cuisine_label').apply(
                lambda x: pd.Series({
            ...
            width = 0.35
            ax.bar(x - width/2, efficacy_summary['mean_pre'], width, label='Pre', color='#74b9ff')
            ax.bar(x + width/2, efficacy_summary['mean_post'], width, label='Post', color='#00b894')
            ...
            plt.savefig(f"{output_dir}/pb2_self_efficacy.png", dpi=150, bbox_inches='tight')
            plt.close()
...
def main():
    ...
    df = pd.DataFrame({
        ...
        'pre_q1': np.random.choice(['1', '2', '3', '99'], n, p=[0.2, 0.5, 0.2, 0.1]),
        'pre_q2': np.random.choice(['1', '2', '3', '99'], n, p=[0.15, 0.55, 0.2, 0.1]),
        'post_q1': np.random.choice(['1', '2', '3', '99'], n, p=[0.1, 0.7, 0.15, 0.05]),
        'post_q2': np.random.choice(['1', '2', '3', '99'], n, p=[0.08, 0.75, 0.12, 0.05]),
    })
