...
from src.cgm_processor import CGMQualityFlags, UnifiedCGMProcessor
...
class TestUnifiedCGMProcessor:
    ...
    def sample_cgm_data(self):
        ...
        glucose = (
            120
            + 20 * np.sin(np.linspace(0, 2 * np.pi, 288))
    ...
    def test_gap_filling_flags_large_gaps(self, processor):
        ...
        result = processor._fill_gaps(df, max_gap_min=15)
        # Large gap should be flagged (not silently interpolated across)
        assert bool(result["gap_flag"].iloc[1]) is True
    ...
    def test_compute_metrics_tir_range(self, processor):
        ...
        # In range 70-180 (inclusive, per ADA): 70, 100, 120, 150, 180 = 5/10 = 50%
        assert metrics["tir"] == 50.0
        assert metrics["tbr"] == 10.0  # <70: 60 = 1/10
        assert metrics["tar"] == 40.0  # >180: 190, 200, 250, 300 = 4/10
...
class TestCGMQualityFlags:
    ...
    def test_default_flags_are_false(self):
        ...
        assert flags.impossible is False
        assert flags.hypoglycemia is False
        assert flags.hyperglycemia is False
        assert flags.missing is False
        assert flags.duplicate is False
